...
class OpenAiProvider @Inject constructor(
    ...
    override val type: ModelProviderType = ModelProviderType.OPENAI

    private val moduleTag = "OpenAiProvider"
    ...
    companion object {
        ...
        private const val DASHSCOPE_BASE = "https://dashscope.aliyuncs.com/compatible-mode/v1"
        private const val ZHIPU_BASE = "https://open.bigmodel.cn/api/paas/v4"
        private const val MOONSHOT_BASE = "https://api.moonshot.cn/v1"
        private const val DEEPSEEK_BASE = "https://api.deepseek.com/v1"
    }
    ...
    private fun detectVendor(config: LlmConfig): String {
        val url = config.baseUrl.lowercase()
        return when {
            url.contains("openai") -> "OpenAI"
            url.contains("dashscope") || url.contains("aliyun") -> "DashScope(通义千问)"
            url.contains("bigmodel") || url.contains("zhipu") -> "智谱AI"
            url.contains("moonshot") -> "Moonshot(Kimi)"
            url.contains("deepseek") -> "DeepSeek"
            url.contains("qwen") -> "Qwen"
    ...
    override suspend fun chat(
        ...
        val vendor = detectVendor(config)

        LingShuLog.i(moduleTag, "[$traceId] chat start | provider=$type | vendor=$vendor | model=${config.modelName} | " +
                "baseUrl=${config.baseUrl} | msgCount=${messages.size} | totalMsgLength=$totalMsgLength | " +
                "estimatedTokens=$estimatedTokens | temp=${config.temperature} | topP=${config.topP} | maxTokens=${config.maxTokens}")
        ...
        while (retryCount <= MAX_RETRIES) {
            try {
                ...
                LingShuLog.d(moduleTag, "[$traceId] HTTP response | vendor=$vendor | code=$httpCode | " +
                        "bodyLength=${responseBodyStr.length} | attempt=${retryCount + 1}")
                ...
                if (lastResponse.isSuccessful) {
                    ...
                    LingShuLog.i(moduleTag, "[$traceId] chat success | vendor=$vendor | httpCode=$httpCode | " +
                    ...
                    LingShuLog.e(moduleTag, "[$traceId] HTTP error | vendor=$vendor | code=$httpCode | " +
                            "attempt=${retryCount + 1} | errorBody=$errorBody")
                    ...
                    return@withContext Result.error(
                        code = errorCode,
                        message = "[$vendor] HTTP $httpCode: ${parseErrorMessage(responseBodyStr)}",
                        cause = null
                    )
                }
            } catch (e: Exception) {
                lastError = e
                LingShuLog.e(moduleTag, "[$traceId] request exception | vendor=$vendor | attempt=${retryCount + 1} | " +
        ...
        lastResponse?.close()
        val elapsed = System.currentTimeMillis() - startTime
        LingShuLog.e(moduleTag, "[$traceId] chat failed after retries | vendor=$vendor | " +
                "totalRetries=$retryCount | elapsedMs=$elapsed", lastError)

        return@withContext Result.error(
            code = ErrorCodes.SERVER_NO_RESPONSE,
            message = "[$vendor] ${lastError?.message ?: "Request failed after $MAX_RETRIES retries"}",
            cause = lastError
        )
    }
    ...
    override suspend fun chatStream(
        ...
        LingShuLog.i(moduleTag, "[$traceId] chatStream start | provider=$type | vendor=$vendor | model=${config.modelName} | " +
        ...
        try {
            ...
            LingShuLog.d(moduleTag, "[$traceId] chatStream HTTP | vendor=$vendor | code=$httpCode")
            ...
            if (!response.isSuccessful) {
                ...
                LingShuLog.e(moduleTag, "[$traceId] chatStream HTTP error | vendor=$vendor | code=$httpCode | errorBody=$truncatedBody")
                response.close()
                return@withContext Result.error(
                    code = if (httpCode == 401) ErrorCodes.API_KEY_INVALID else ErrorCodes.SERVER_NO_RESPONSE,
                    message = "[$vendor] HTTP $httpCode: ${parseErrorMessage(errorBody)}"
                )
            }
            ...
            response.close()
            val elapsed = System.currentTimeMillis() - startTime
            LingShuLog.i(moduleTag, "[$traceId] chatStream success | vendor=$vendor | httpCode=$httpCode | " +
            ...
            val elapsed = System.currentTimeMillis() - startTime
            LingShuLog.e(moduleTag, "[$traceId] chatStream exception | vendor=$vendor | elapsedMs=$elapsed | " +
                    "type=${e.javaClass.simpleName} | msg=${e.message}", e)
            return@withContext Result.error(
                code = ErrorCodes.SERVER_NO_RESPONSE,
                message = "[$vendor] ${e.message ?: "Stream failed"}",
                cause = e
            )
        }
    }
    ...
    override suspend fun embeddings(
        ...
        LingShuLog.i(moduleTag, "[$traceId] embeddings start | provider=$type | vendor=$vendor | model=${config.modelName} | " +
        ...
        try {
            ...
            LingShuLog.d(moduleTag, "[$traceId] embeddings HTTP | vendor=$vendor | code=$httpCode | bodyLength=${responseBodyStr.length}")
            ...
            if (!response.isSuccessful) {
                ...
                LingShuLog.e(moduleTag, "[$traceId] embeddings HTTP error | vendor=$vendor | code=$httpCode | errorBody=$truncatedBody")
                response.close()
                return@withContext Result.error(
                    code = if (httpCode == 401) ErrorCodes.API_KEY_INVALID else ErrorCodes.SERVER_NO_RESPONSE,
                    message = "[$vendor] HTTP $httpCode: ${parseErrorMessage(responseBodyStr)}"
            ...
            LingShuLog.i(moduleTag, "[$traceId] embeddings success | vendor=$vendor | httpCode=$httpCode | " +
                    "vectors=${result.size} | dims=${result.firstOrNull()?.size ?: 0} | elapsedMs=$elapsed")

            return@withContext Result.success(result)
        } catch (e: Exception) {
            val elapsed = System.currentTimeMillis() - startTime
            LingShuLog.e(moduleTag, "[$traceId] embeddings exception | vendor=$vendor | elapsedMs=$elapsed | " +
                    "type=${e.javaClass.simpleName} | msg=${e.message}", e)
            return@withContext Result.error(
                code = ErrorCodes.SERVER_NO_RESPONSE,
                message = "[$vendor] ${e.message ?: "Embeddings failed"}",
                cause = e
            )
        }
    }
    ...
    override fun isAvailable(config: LlmConfig): Boolean {
        ...
        if (!valid) {
            LingShuLog.w(moduleTag, "isAvailable=false | apiKeyBlank=${config.apiKey.isBlank()} | " +
                    ...
                    "vendorHint=${detectVendor(config)}")
        } else {
            LingShuLog.d(moduleTag, "isAvailable=true | vendor=${detectVendor(config)} | model=${config.modelName}")
        }
        return valid
    }
